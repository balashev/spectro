...
class Doublet_saved():
    ...
    def draw(self):
        self.determineY()
        self.line1 = pg.InfiniteLine(self.l1 * (1 + self.z), angle=90,
                                     pen=pg.mkPen(color=self.color, width=0.5, style=Qt.SolidLine))
        self.parent.vb.addItem(self.line1)
        self.ind1 = doubletLabel(self, self.name, self.l1, angle=90, anchor=(0,1), color=self.color)
        self.parent.vb.addItem(self.ind1)
        self.line2 = pg.InfiniteLine(self.l2 * (1 + self.z), angle=90,
                                     pen=pg.mkPen(color=self.color, width=0.5, style=Qt.SolidLine))
        self.parent.vb.addItem(self.line2)
        self.ind2 = doubletLabel(self, self.name, self.l2, angle=90, anchor=(0,0), color=self.color)
        self.parent.vb.addItem(self.ind2)

    def redraw(self):
        self.determineY()
        self.line1.setPos(self.l1 * (1 + self.z))
        self.ind1.redraw()
        self.line2.setPos(self.l2 * (1 + self.z))
        self.ind2.redraw()

    def remove(self):
        self.parent.vb.removeItem(self.line1)
        self.parent.vb.removeItem(self.ind1)
        self.parent.vb.removeItem(self.line2)
        self.parent.vb.removeItem(self.ind2)
