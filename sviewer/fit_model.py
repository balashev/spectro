        ...
        if self.sort.isChecked():
            ...
            for el in ['H2', 'CO', 'HD', 'CI', 'SiII']:
                if any([el in sp for sp in sps.keys()]):
                    ind = []
                    for i, sp in enumerate(sps.keys()):
                        if el == sp:
                            ind.append([0, i])
                        elif el in sp:
                            if 'v' not in sp:
                                ind.append([int(sp[sp.index('j')+1:]), i])
                            else:
                                ind.append([int(sp[sp.index('j')+1:sp.index('v')]) + 20 * int(sp[sp.index('v')+1:]), i])
                    print(ind)
...
class chooseShowParsWidget(QWidget):
    ...
    def click(self, s):
        self.parent.fit.setValue(s, getattr(self, s).isChecked(), attr='show')
